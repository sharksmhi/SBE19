...
===============================================================================
NEW COMMENTS
UPDATED 2025-05-10, Added new SLU SBE19 to processing. (8438)
...
#            self.modify_cnv_file()
    ...
    def load_options(self):
        ...
        self.plot_directory = 'C:\\ctdSBE19\\data_local\\20' + self.year +' \\plots\\'
...
#----------------------------------------------------------------------
        ...
        #Seabird 19    SMHIs
        if self.ctdNo == '6164' or self.ctdNo == '8438':
            #OBS! Om något ändrats på CTDn, ex bytt sensorer, kör Seabirds program manuellt och titta på cnv-filen och uppdatera formatet nedan.
            #   [col,   'format',   'parameter name']            
    ...
    def create_batch_file(self):
        ...
        elif self.ctdNo in ['6164','6929', '8438']: 
        ...
        #Cell thermal mass
        if self.ctdNo in ['6164','6929', '8438']: 
    ...
    def modify_cnv_file(self):        
        ...
        try:
            ...
            print('No metadata to file header - SBE19?')
            pass
        ...
        for i,x in enumerate(self.ctd_data[0]):
            ...
            if 'par: PAR/Irradiance' in x:                                  
                self.ctd_data[0][i] = self.ctd_data[0][i][:-1] + ' [µE/(m^2*s)]\n'
                #self.ctd_data[0][i] = self.ctd_data[0][i][:-1] + ' [\xB5E/(m^2*s)]\n'
                #self.ctd_data[0][i] = self.ctd_data[0][i][:-1] + ' [\u00B5E/(m^2*s)]\n'
        ...
        filelist=next(os.walk(self.data_directory +'20' + self.year + '\cnv\\'))[2]
...
#            
            ...
            q=input('Files do already exist. Overwrite? Y or N?')
            # q = 'Y'
            if q.upper() == 'Y': # om Y; skriv över filen
                #Skriver tillbaka header self.ctd_data[0], 
                test_file = open(self.data_directory + '20' + self.year + '\\cnv\\' + self.new_fname + '.cnv', 'w')
                print((self.ctd_data[0]))                
                #test_file.writelines(self.ctd_data[0])
                test_file.writelines(self.ctd_data[0])                
                test_file.close()
    ...
    def copy_files_to_server(self):
        ...
        if self.cnty == '77' and (self.ship == '10' or self.ship == 'SE'):
            ...
            if 0:
                ...
                server_asvp_dir = '\\\\scifi01.svea.slu.se\\Temp\\ASVP\\'
            ...
            try:
                ...
                #print filename
                #print self.data_directory + '\\20' + self.year + '\\plots\\' + filename
                #print server_plot_dir + filename
            except:
                print(filename)
                print(self.data_directory + '\\20' + self.year + '\\plots\\' + filename)
                print(server_plot_dir + filename)                
            ...
            # ASVP for eco-sounder
            try:
                shutil.copyfile(self.data_directory + '\\ASVP\\' + self.new_fname + '.asvp',
                                server_asvp_dir + self.new_fname + '.asvp')
            except:
                print('Cant copy ASVP file to file server!')
...
#            print row
            ...
            if row[index] == -9.990e-29 or sensor_flag == 0:
                text_line = ''.join([text_line, ' -9.990e-29'])
            else:
    ...
    # ==========================================================================
    def make_ASVP_file(self):
        print('Making ASVP-file from cnv-file...')

        # Sökväg till filen där redigerade filer hamnar
        myFileDir = self.data_directory + '/ASVP'
        ...
        file_path = self.data_directory + '\\20' + self.year + '\\cnv\\' + self.new_fname + '.cnv'
        ...
        lat = None
        lon = None
        file_time_str = names[11:24].replace("_","")
        file_time = datetime.datetime.strptime(file_time_str, "%Y%m%d%H%M")

        for i, line in enumerate(lines):

            if "** LIMS Job:" in line:
                serno = line.split(":")[1].strip()

            if "** Latitude:" in line:
                lat_str = line.split(":")[1].replace("N", "").strip()
                deg, minutes = lat_str.split()
                lat = float(deg) + float(minutes) / 60

            if "** Longitude:" in line:
                lon_str = line.split(":")[1].replace("E", "").strip()
                deg, minutes = lon_str.split()
                lon = float(deg) + float(minutes) / 60

            #if "NMEA UTC (Time)" in line:
            #    t = line.split("=")[1].strip()
            #    file_time = datetime.datetime.strptime(t, "%b %d %Y %H:%M:%S")
        ...
        valid_from = file_time.strftime("%Y%m%d%H%M")
        valid_to = file_time.strftime("%Y%m%d%H%M")

        nr_values = len(df)

        header = f"( SoundVelocity {version} {serno} {time_str} {lat:.6f} {lon:.6f} {radii} {valid_from} {valid_to} {src} {hist} {nr_values} )"

        # -----------------------
        # skriv fil
        # -----------------------

        #with open(myFileDir + '\\' + names[0:-4] + '.asvp', "w") as f:
        with open(myFileDir + '\\' + names + '.asvp', "w") as f:

            f.write(header + "\n")

            for zz, vv in zip(z, vel):
                f.write(f"{zz:.2f} {vv:.2f}\n")

        print("ASVP-fil skapad.")

    #==========================================================================
    def make_LIMS_export_file(self)        :
        print('Making LIMS-export from cnv-file...')    
        #Header of LIMS-export file
        header = 'Depth [m],','Temp. [deg C],','Sal. [ ],','Chl-Flu. [µg/l],','Turbidity [ ],','Oxygen [ml/l],','Flag'
       ...
       #Hitta första maxvärde för djup och korta av value...
        ...
        end_file.write('Uppgifter från från provtagningsprotokoll\n')
        end_file.write('Station: ' + station + '\n')
        end_file.write('Latitud: ' + latitud + '\n')
        end_file.write('Longitud: ' + longitud + '\n')
        end_file.write('Datum: ' + names[11:15]+ '-' + names[15:17] + '-' + names[17:19] + '\n')        
        end_file.write('Tid: ' + names[20:22]+ ':' + names[22:24] + '\n')
        
        # Add LIMS Job
        end_file.write('LIMS Job: ' + names.split('_')[2][0:4] + names.split('_')[4] + names.split('_')[5] + '-' + names.split('_')[6].strip('.cnv') + '\n\n')
    ...
    def get_string_for_shark_file(self, row=False, header=False):
        ...
        text_list = list(range(len(shark_index)))
        header_list = list(range(len(shark_index)))
